...
class User(Base):
    ...
    mpg = Column(Integer, primary_key=True)
    cylinder = Column(integer)
    cubicinches = Column(integer)
    hp = Column(integer)
    weightlbs=column(Integer)
    timeto60=column(Integer)
    year=column(Integer)
    brand=Column(string)

def __ (self):
    return "<User(mpg='%s', cylinder='%s', cubicinches='%s', hp='%s', weightlbs='%s', timeto60='%s',year='%s',brand='%s')>" % (self.name, self.fullname, self.nickname)
